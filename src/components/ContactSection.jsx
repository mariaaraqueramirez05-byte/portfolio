...
const socialLinks = [
  {
    name: 'Behance',
    href: 'https://www.behance.net/gabrieltakafum',
    icon: (
  ...
  {
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/in/mariagabrielaaraqueramirez/',
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
  ...
  {
    name: 'Instagram',
    href: 'https://www.instagram.com/wh01s_maria?igsh=MWQwcDBzenhncmFrNA%3D%3D&utm_source=qr',
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://formsubmit.co/ajax/mariaaraqueramirez05@gmail.com', {
        method: 'POST',
      ...
      if (res.ok) {
        setStatus({ type: 'success', text: '¡Mensaje enviado con éxito! Te responderé pronto.' })
        setFormData({ name: '', email: '', message: '' })
      } else {
        setStatus({ type: 'error', text: 'Hubo un error. Intenta de nuevo o escríbeme directamente al correo.' })
      }
    } catch {
      setStatus({ type: 'error', text: 'Hubo un error. Intenta de nuevo o escríbeme directamente al correo.' })
    } finally {
          ...
          >
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full px-8 py-3.5 bg-primary text-white rounded-xl font-medium text-sm hover:bg-primary-dark transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed"
              >
          ...
          >
            <div>
              ...
              <div className="space-y-4 mb-8">
                <a
                  href="mailto:mariaaraqueramirez05@gmail.com"
                ...
                >
                  ...
                  mariaaraqueramirez05@gmail.com
                </a>
              </div>
            </div>
